...
let nowdate = moment();
let news = '';
const cardsHTML = info.forEach(item => {
    if(moment(item.date).isBefore(nowdate)) {
        news += `<div class="new">
        <h2 class="title_for_new" data-articul="${info.indexOf(item)}">${item.title}</h2> 
        <h3 class="text_for_new"  data-articul="${info.indexOf(item)}">${item.text}</h3>
        <h3 class="date_for_new"  data-articul="${info.indexOf(item)}">${item.date}</h3>
        </div>`;
    }
});
...
changer.addEventListener(('click'), event => {
    if (event.target.classList.contains('title_for_new')) {
        ...
        // date1[articul].classList.toggle("display_block");
